pipeline {
    ...
    steps {
        ...
        sshagent(credentials: ['application-server-creds']) {
            ...
            sh "scp -o StrictHostKeyChecking=no target/my-app-1.0-SNAPSHOT.jar azureuser@52.136.127.164:/home/simple-maven-app/new"
        }

        // Trigger build process on remote server using IP address and username/password
        sshagent(credentials: ['application-server-creds']) {
            sh 'ssh azureuser@52.136.127.164 "cd /home/simple-maven-app/new/target/; java -jar my-app-1.0-SNAPSHOT.jar"'
        }
    }
}
